...
def extract_ktp_data(text_response, debug_mode=False):
    ktp_extract = pd.DataFrame(
        columns=['province', 'city', 'identity_number', 'fullname', 'birth_place', 'birth_date', 'nationality',
                 'occupation', 'gender', 'marital_status',
    ...
    for field in fields_ktp:
        field_value = get_attribute_ktp(ls_word, field['field_name'], field['keywords'], field['typo_tolerance'],
                                        debug_mode)
        if (field_value != None):
    ...
    if (attributes['birth_place'] != None):
        attributes['birth_place'] = ''.join([i for i in attributes['birth_place'] if not i.isdigit()]).replace('-',
                                                                                                               '').replace(
            '.', '').strip()
